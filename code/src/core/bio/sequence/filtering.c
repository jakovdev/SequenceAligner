...
static float similarity_pairwise(sequence_ptr_t seq1, sequence_ptr_t seq2)
{
	if (UNLIKELY(!seq1->length || !seq2->length))
		return 0.0f;

	sequence_length_t min_len = seq1->length < seq2->length ? seq1->length :
								  seq2->length;
	size_t matches = 0;
		      ...
		      sequence_count_t *filtered_count)
{
	if (!sequences || !keep_flags || !filtered_count) {
		print(M_NONE, ERROR "Invalid parameters to filter sequences");
		return false;
	}

	if ((sequence_count <= 2) || (filter_threshold <= 0.0f) ||
	    (filter_threshold > 1.0f)) {
		print(M_NONE,
		      ERROR "Invalid sequence count or filter threshold");
		return false;
	}
	...
	*filtered_count = 0;
	...
	const size_t progress_update_interval =
		expected_progress / (num_threads * 100);
...
#pragma omp for schedule(dynamic)
		for (sequence_count_t i = 1; i < sequence_count; i++) {
			...
			for (sequence_count_t j = 0; j < i; j++) {
				...
				float similarity = similarity_pairwise(
