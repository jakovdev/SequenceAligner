...
void affine_local_init(score_t *restrict match, score_t *restrict gap_x,
...
score_t affine_local_fill(score_t *restrict match, score_t *restrict gap_x,
			  ...
			  sequence_ptr_t seq2)
{
	...
	for (sequence_length_t i = 1; i <= len2; ++i) {
		...
		for (sequence_length_t j = 1; j <= len1; j++) {
			score_t similarity =
				SCORING_MATRIX[seq1_idx[j - 1]][c2_idx];
			score_t diag_score =
