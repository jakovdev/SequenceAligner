...
score_t align_sw(sequence_ptr_t seq1, sequence_ptr_t seq2)
{
	...
	size_t matrices_size = USE_STACK_MATRIX(matrices_bytes) ?
				       MATRICES_3X_SIZE(len1, len2) :
				       1;
#ifdef _MSC_VER
...
no_stack:
	...
	score_t score = affine_local_fill(match, gap_x, gap_y, &seq1_indices,
					  seq1, seq2);
