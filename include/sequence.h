...
INLINE SequenceData load_sequences_from_file(char* current, char* end, size_t total_seqs_in_file) {
    ...
    print_success(apply_filtering ? "Loaded %zu sequences (filtered out %zu)" : "Loaded %zu sequences", seq_count, apply_filtering ? filtered_count : 0);
