...
INLINE void set_terminal_raw_mode(void) {
    #ifdef _WIN32
...
INLINE void restore_terminal_mode(void) {
    #ifdef _WIN32
    HANDLE hStdin = GetStdHandle(STD_INPUT_HANDLE);
    DWORD mode;
    GetConsoleMode(hStdin, &mode);
...
INLINE void read_line_input(char* input_buffer, int buffer_size, int* choice, const char* prompt) {
    int idx = 0;
    int c;
