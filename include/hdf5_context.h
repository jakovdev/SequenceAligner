...
h5_create_file(void)
{
    ...
    g_hdf5.file_id = H5Fcreate(args_path_output(), H5F_ACC_TRUNC, H5P_DEFAULT, fapl);
    H5Pclose(fapl);
...
h5_create_sequence_group(void)
{
    ...
    if (seq_group < 0)
    {
        ...
        H5Dclose(g_hdf5.matrix_dataset_id);
        g_hdf5.matrix_dataset_id = -1;
        H5Fclose(g_hdf5.file_id);
        g_hdf5.file_id = -1;
        return false;
    }
...
h5_create_sequence_length_dataset(void)
{
    ...
    hid_t seq_lengths_space = H5Screate_simple(1, g_hdf5.seq_dims, NULL);
    ...
    g_hdf5.seq_lengths_dataset_id = H5Dcreate2(g_hdf5.file_id,
                                               "/sequences/lengths",
                                               H5T_STD_U64LE,
                                               seq_lengths_space,
                                               H5P_DEFAULT,
    ...
    if (g_hdf5.seq_lengths_dataset_id < 0)
    {
...
h5_initialize_memory(void)
{
    if (g_hdf5.use_mmap)
    {
        mmap_matrix_file_name(g_hdf5.mmap_filename, MAX_PATH, args_path_output());
...
h5_cleanup_on_init_failure(void)
{
    ...
    if (g_hdf5.seq_lengths_dataset_id > 0)
    {
        H5Dclose(g_hdf5.seq_lengths_dataset_id);
    }
...
{
    g_hdf5.matrix_size = matrix_size;
    g_hdf5.file_id = -1;
    g_hdf5.matrix_dataset_id = -1;
    g_hdf5.seq_dataset_id = -1;
    g_hdf5.seq_lengths_dataset_id = -1;
...
h5_flush_mmap_to_hdf5(void)
{
    ...
    hid_t file_space = H5Dget_space(g_hdf5.matrix_dataset_id);
    if (file_space < 0)
    {
        free(buffer);
        return false;
...
{
    if (!args_mode_write() || !g_hdf5.is_init)
    {
        return true;
    }

    if (g_hdf5.matrix_dataset_id < 0 || (g_hdf5.use_mmap && !g_hdf5.mmap_matrix.data) ||
...
{
    if (!args_mode_write() || !g_hdf5.is_init || row >= g_hdf5.matrix_size ||
...
h5_store_sequence_lengths(sequence_t* sequences, size_t seq_count)
{
    ...
    herr_t status = H5Dwrite(g_hdf5.seq_lengths_dataset_id,
                             H5T_NATIVE_ULONG,
                             H5S_ALL,
                             H5S_ALL,
                             H5P_DEFAULT,
                        ...
                        hid_t seq_space)
{
    ...
    status = H5Dwrite(g_hdf5.seq_dataset_id,
                      string_type,
                      batch_mem_space,
                      seq_space,
                      H5P_DEFAULT,
...
h5_create_sequence_dataset(hid_t string_type)
{
    ...
    g_hdf5.seq_dataset_id = H5Dcreate2(g_hdf5.file_id,
                                       "/sequences/data",
                                       string_type,
                                       seq_space,
                                       H5P_DEFAULT,
...
{
    if (!args_mode_write() || !g_hdf5.is_init || g_hdf5.sequences_stored)
    {
        return true;
    }
    ...
    for (size_t batch_start = 0; batch_start < seq_count; batch_start += batch_size)
    {
        ...
        if (!h5_store_sequence_batch(sequences, batch_start, batch_end, string_type, seq_space))
        {
            H5Sclose(seq_space);
            H5Tclose(string_type);
            H5Dclose(g_hdf5.seq_dataset_id);
            g_hdf5.seq_dataset_id = -1;
            return false;
        }
...
{
    if (!args_mode_write() || g_hdf5.matrix_dataset_id < 0 || g_hdf5.file_id < 0)
    {
        return false;
    }

    htri_t attr_exists = H5Aexists(g_hdf5.matrix_dataset_id, "checksum");
    if (attr_exists > 0)
    {
        H5Adelete(g_hdf5.matrix_dataset_id, "checksum");
    }
    ...
    hid_t attr_id = H5Acreate2(g_hdf5.matrix_dataset_id,
                               "checksum",
                               H5T_STD_I64LE,
                               attr_space,
                               H5P_DEFAULT,
