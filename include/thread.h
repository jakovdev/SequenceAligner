...
thread_pool_worker(void* arg)
{
    ...
    while (1)
    {
        ...
        size_t task_block;
        while (
            (task_block = __atomic_fetch_add(&g_task_counter, TASK_BLOCK_SIZE, __ATOMIC_RELAXED)) <
...
#pragma GCC unroll 8
            for (size_t task_idx = task_block; task_idx < block_end; task_idx++)
            {
                ...
                h5_set_matrix_value(g_h5_handler, task->i, task->j, score);
                h5_set_matrix_value(g_h5_handler, task->j, task->i, score);
            }
        }

        __atomic_fetch_add(&g_h5_handler->checksums[thread_id], local_checksum, __ATOMIC_RELAXED);
        local_checksum = 0;
...
init_thread_pool(H5Handler* h5_handler)
{
    ...
    g_h5_handler->checksums = aligned_alloc(CACHE_LINE,
                                            sizeof(*g_h5_handler->checksums) * g_num_threads);

    for (int i = 0; i < g_num_threads; i++)
    {
        g_h5_handler->checksums[i] = 0;
    }
                   ...
                   const ScoringMatrix* scoring)
{
    if (get_mode_multithread())
    {
        ...
        double clamped_ratio = log_seq_ratio < 0.0 ? 0.0
                                                   : (log_seq_ratio > 1.0 ? 1.0 : log_seq_ratio);

        double optimal_memory = ref_small_bytes +
                                clamped_ratio * (ref_large_bytes - ref_small_bytes);

        size_t optimal_batch_size = (size_t)(optimal_memory / task_size);
        optimal_batch_size = optimal_batch_size < total_alignments ? optimal_batch_size
                                                                   : total_alignments;
