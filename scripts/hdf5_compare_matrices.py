        ...
        for i in range(0, shape[0], chunk_size):
            ...
            for j in range(0, shape[1], chunk_size):
                ...
                for idx in range(len(diff_indices[0])):
                    ...
                    print(f"  {file1}: {chunk1[diff_indices[0][idx], diff_indices[1][idx]]}")
                    print(f"  {file2}: {chunk2[diff_indices[0][idx], diff_indices[1][idx]]}")
...
def parse_args():
    parser = argparse.ArgumentParser(description='Compare similarity matrices in two HDF5 files.')
